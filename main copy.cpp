...
//  ITSC3146_6_2
//
//  Created by Tyler Flowers on 9/25/23.
//
#include <pthread.h>
#include <iostream>
#include <unistd.h>
...
void* takeOrders(void* arg)
{
    ...
    for(int i = 0; i < MAX; ++i) {
        ...
        //mutex statement that allows access
        ...
        //mutex statement that relinquishes access
        pthread_mutex_unlock(&console_mutex);
    
        // Beginning of critical region 2
        //mutex statement that allows access && if statement that triggers not_full
        ...
        //mutex statement that relinquishes access and cond statement that signals not_empty
...
void* processOrders(void* arg)
{
    ...
    for(int i = 0; i < MAX; ++i) {
        ...
        //mutex statement that allows access && if statement that triggers not_empty
        pthread_mutex_lock(&data_mutex);
        if(num_new_orders == 0)
        ...
        //mutex statement that relinquishes access and cond statement that signals not_full
        pthread_cond_signal(&not_full);
        pthread_mutex_unlock(&data_mutex);

        
        // Beginning of critical region 4
        //mutex statememt that allows acess to console_mutex
        pthread_mutex_lock(&console_mutex);
        ...
        //mutex statement that relinquishes acess to console_mutex
        pthread_mutex_unlock(&console_mutex);
...
int main()
{
    ...
    //code that waits for both threads to finish
